...
class jTendik{
    ...
    public function gettendikById($id){
        ...
        $query = $this->db->prepare("SELECT t.jawaban_tendik_id, t.responden_tendik_id, s.soal_pertanyaan, t.jawaban, k.kategori_nama, DATE_FORMAT(t.responden_tanggal, '%d-%m-%Y %H:%i:%s') AS responden_tanggal
        FROM t_jawaban_mahasiswa as t
        FROM t_jawaban_tendik as t
        JOIN m_survey_soal as s ON s.soal_id = t.soal_id
        JOIN m_kategori as k ON s.kategori_id = k.kategori_id
        WHERE t.responden_tendik_id = ?");
